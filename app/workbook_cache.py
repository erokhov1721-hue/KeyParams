...
# Сводка по всем объектам сразу (инвесторская страница, сравнение со средним
# по классу) открывает не один и тот же файл несколько раз, а разные файлы
# по одному разу каждый — но по всему портфелю сразу: на 13.09.2026 это 26
# файлов по два варианта (data_only True/False) — 52 записи. 8 (расчёт под
# первый сценарий — одна и та же смета несколько раз на странице объекта) не
# хватало даже на один такой проход: кэш проворачивался быстрее, чем успевал
# пригодиться, и портфельные страницы каждый раз читали всё с диска заново
# (~35 секунд на 26 файлов). 256 — с запасом на рост портфеля, не только под
# сегодняшнее число объектов.
MAX_ENTRIES = 256
