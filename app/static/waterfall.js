...
(function (global) {
  ...
  // сюда одну опорную строку («Смета») и две пары («Подписанное
  // удорожание» → «с учётом подписанного», «Прогнозируемое удорожание» →
  // «Итоговая стоимость»); тестовые — «тендер»+«ДГП» опорными и МР/Альфа
  // парами. Опоры нет вовсе (как «Прочее» без тендера и ДГП) — тогда
  // «deltas» превращаются в самостоятельные итоговые строки без дельты.
  function stepsOf(group) {
    ...
    (group.deltas || []).forEach(function (d) {
      if (hasBase) {
  ...
  function renderWaterfall(container, groups, unit) {
    ...
    rows.forEach(function (row) {
      ...
      // Смета — нейтральный серый; любая дельта («удорожание») — акцентный
      // красный (тот же цвет, что и «стало хуже» в KPI над графиком, — рост
      // сметы это тоже «хуже»); нарастающий итог после дельты — приглушённый
      // фиолетовый. Раньше «Подписанное удорожание» и «с учётом подписанного
      // удорожания» красились в один и тот же цвет группы (мира/данжер) и
      // были неотличимы друг от друга — теперь цвет зависит от типа строки
      // (дельта/итог), а не от того, какая это по счёту пара.
